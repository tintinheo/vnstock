...
# ── Python version guard (L-01: requires 3.10+ for union type hints) ──────────
if sys.version_info < (3, 10):
    raise RuntimeError(
        f"Python 3.10+ required for quant_terminal. "
...
from config import (
    ...
    APP_VERSION, WATCHLIST_DEFAULT, compute_true_breakeven, VN_GRAHAM_MULTIPLIER
)
...
from modules.market_intel import VN30_SECTORS, compute_sector_returns, compute_market_breadth, get_fol_status, SECTOR_BETA_DISCLAIMER_VI
...
def fmt_vnd(v, decimals=0):
    """Format a raw-VND value (thousands-VND × 1000) to display string."""
    if v is None or (isinstance(v, float) and math.isnan(v)):
        return "—"
    return f"{v*1000:,.{decimals}f}đ"
...
with st.sidebar:
    ...
    # ── Legal Disclaimer (C-07) ───────────────────────────────────────────────
    with st.expander("⚠️ Tuyên bố miễn trách nhiệm pháp lý", expanded=False):
        st.markdown("""
**⚠️ TUYÊN BỐ MIỄN TRÁCH NHIỆM PHÁP LÝ**

Ứng dụng này là **CÔNG CỤ PHÂN TÍCH KỸ THUẬT** tự động, **KHÔNG PHẢI** dịch vụ tư vấn đầu tư
được cấp phép theo Luật Chứng khoán 2019 và Thông tư 09/2023/TT-BTC.

Mọi tín hiệu, gợi ý và phân tích chỉ mang tính tham khảo kỹ thuật thuần túy.
Nhà đầu tư phải **tự chịu hoàn toàn trách nhiệm** về quyết định đầu tư.

Tham khảo tư vấn đầu tư được cấp phép trước khi ra quyết định quan trọng.

Dữ liệu danh mục được xử lý **cục bộ** trên máy tính này — không gửi lên bất kỳ máy chủ nào.
        """)
    ...
    with st.expander("⚙️ Cấu hình"):
        st.info("📡 Nguồn dữ liệu: SSI iBoard (real-time)")
...
with tab_dashboard:
    ...
    with c5:
        ...
        st.metric("Mã GD được hôm nay", len(tradeable),
                  help="Số mã đã T+2 (thanh toán hoàn tất) — có thể bán ngay. "
                       "⏳ = mua chưa đủ T+2, chờ đến ngày thanh toán mới bán được.")
    ...
    if not pf.df.empty:
        ...
        for _, row in pf.df.iterrows():
            ...
            t2_ok   = "✅" if tradeable_qty and float(tradeable_qty) > 0 else "⏳ Chờ T+2"
            cost_p  = float(row.get("cost_price", 0) or 0)
            breakeven = compute_true_breakeven(cost_p) if cost_p > 0 else 0.0
            disp_cols.append({
                "Mã CK": row["symbol"],
                "KL": int(float(row.get("total_qty", 0))),
                "T+2": t2_ok,
                "Giá vốn": fmt_price(cost_p),
        ...
        st.caption("Break-even = Giá vốn / (1 − 0.1% thuế bán). Mức giá cần bán để hoà vốn.")

        # Margin warning (H-07)
        st.info(
            "ℹ️ Nếu bạn mua ký quỹ (margin/TTM), giá vốn trong file SSI là giá toàn phần "
            "(đã bao gồm phần vay). P&L tính đúng, nhưng cần theo dõi thêm mức margin call / "
            "force-sell từ SSI. Liên hệ SSI để biết mức tỷ lệ ký quỹ duy trì."
        )
...
with tab_stock:
    ...
    # ── FOL (Foreign Ownership Limit) warning (H-04) ─────────────────────────
    if symbol:
        _foreign_pct = quote.get("foreign_buy", 0) / max(qty, 1) * 100 if qty > 0 else 0
        _fol = get_fol_status(symbol, _foreign_pct)
        if _fol.get("warning"):
            st.markdown(
                f'<div class="alert-warn">{_fol["warning"]}</div>',
                unsafe_allow_html=True,
            )
    ...
    with col_sig:
        ...
        if not hist.empty:
            ...
            if financials:
                ...
                # ── Fundamental valuation (C-01 / C-05) ─────────────────────
                eps_raw  = financials.get("eps", 0.0)   # raw VND per share
                bvps_raw = 0.0   # not available from this endpoint — show N/A

                if eps_raw > 0:
                    eps_k    = eps_raw / 1000.0   # thousands-VND per share
                    pb       = financials.get("pb", 0)
                    pe       = financials.get("pe", 0)
                    # IV via P/E method (real EPS from SSI)
                    iv_pe    = round(eps_k * pe, 2) if pe > 0 else None
                    # Graham Number: √(22.5 × EPS_k × BVPS_k) — requires BVPS
                    # If P/B available and market price known, derive BVPS
                    if pb > 0 and market_price > 0:
                        bvps_k   = market_price / pb
                        iv_graham= round((VN_GRAHAM_MULTIPLIER * eps_k * bvps_k) ** 0.5, 2)
                    else:
                        iv_graham = None

                    st.markdown("---")
                    st.markdown('<div style="font-size:11px;color:#6B7280;font-weight:600;">ĐỊNH GIÁ CƠ BẢN</div>', unsafe_allow_html=True)
                    if iv_pe:
                        diff_pct = (market_price / iv_pe - 1) * 100 if iv_pe > 0 else 0
                        _color   = "#DC2626" if diff_pct > 15 else ("#16A34A" if diff_pct < -10 else "#6B7280")
                        st.markdown(
                            f'<div style="font-size:12px;">P/E Fair Value: '
                            f'<b>{fmt_price(iv_pe)}</b> '
                            f'<span style="color:{_color};">({diff_pct:+.1f}% vs TT)</span></div>',
                            unsafe_allow_html=True
                        )
                    if iv_graham:
                        diff_g = (market_price / iv_graham - 1) * 100 if iv_graham > 0 else 0
                        _cg    = "#DC2626" if diff_g > 15 else ("#16A34A" if diff_g < -10 else "#6B7280")
                        st.markdown(
                            f'<div style="font-size:12px;">Graham Number*: '
                            f'<b>{fmt_price(iv_graham)}</b> '
                            f'<span style="color:{_cg};">({diff_g:+.1f}% vs TT)</span></div>',
                            unsafe_allow_html=True
                        )
                    st.caption(
                        "* Graham Number dùng hệ số 22.5 (hiệu chỉnh cho VN P/E ~13× × P/B ~1.75×). "
                        "Công thức gốc thiết kế cho thị trường Mỹ thập niên 1960. "
                        "Chỉ mang tính tham khảo — không phải giá trị nội tại chính xác."
                    )
                else:
                    st.caption("EPS chưa có — định giá cơ bản không khả dụng.")
        else:
            ...
            if "macd_hist" in df_ind.columns:
                ...
                if "macd_signal" in df_ind.columns:
                    fig.add_trace(go.Scatter(
                        ...
                        line=dict(color="#F59E0B", width=1.2, dash="dot"), showlegend=False
                    ), row=3, col=1)
    ...
    analyst_target = st.number_input(
        "Giá mục tiêu analyst (nghìn đ, để trống = 0)",
        min_value=0.0, value=0.0, step=0.05, format="%.2f",
        help="Nhập bằng ngàn đồng (VD: 30.0 = 30,000đ)"
    )
    ...
    if not hist.empty and cost_price > 0:
        ...
        st.caption(
            "ℹ️ Thuế 0.1% SSI khấu trừ tại nguồn khi bán. Nhà đầu tư vẫn có nghĩa vụ kê khai "
            "quyết toán thuế TNCN hàng năm nếu phát sinh thu nhập từ chứng khoán "
            "(TT 92/2015/TT-BTC). Trong thực tế với mức 0.1% cố định, thường không phát sinh thêm."
        )
        ...
        with lo_c3:
            if lo_order_type == "LO":
                lo_price = st.number_input(
                    "Giá đặt (nghìn đ)",
                    min_value=0.0,
                    value=float(market_price or cost_price or 15.0),
                    step=0.05,
                    format="%.2f",
                    help="Giá tính bằng ngàn đồng (VD: 26.65 = 26,650 đ). Bước nhảy 0.05 = 50 VND"
                )
            else:
...
with tab_market:
    ...
    if not st.session_state.portfolio.df.empty and not vni_hist.empty:
        ...
        st.caption(
            "ℹ️ Alpha được tính so với VNINDEX của 120 phiên gần nhất từ dữ liệu file SSI — "
            "không phải alpha điều chỉnh rủi ro. Tham khảo thêm Sharpe/Sortino ở tab Rủi Ro."
        )
    ...
    if _sector_rets:
        ...
        st.caption(SECTOR_BETA_DISCLAIMER_VI)
    else:
...
with tab_risk:
    ...
    if pf.df.empty:
        ...
        if var_rows:
            ...
            st.caption(
                "ℹ️ VaR danh mục ở đây là tổng bình quân gia quyền — phương pháp bảo thủ, "
                "bỏ qua lợi ích phân tán rủi ro (diversification). VaR thực tế thường thấp hơn "
                "khi các vị thế ít tương quan. Tham khảo ma trận tương quan bên dưới."
            )
