...
def _fetch_ohlcv_ssi(ticker: str, start: date, end: date) -> pd.DataFrame:
    ...
    try:
        raw = _get(url, params, timeout=20, raise_errors=True)
        if not raw:
            return pd.DataFrame()
...
def _ohlcv_result(
    ...
    freshness = {
        CapabilityStatus.STALE_CACHE: "STALE",
        CapabilityStatus.MISSING: "UNAVAILABLE",
        CapabilityStatus.FETCH_FAILED: "UNAVAILABLE",
        CapabilityStatus.NOT_CONFIGURED: "UNAVAILABLE",
        CapabilityStatus.DQ_FAILED: "INVALID",
    }.get(status, "FRESH")
