...
class ScannerService:
    ...
    def _score_ticker(
        ...
        if df.empty or len(df) < 40:
            _, actionability = apply_publication_gate(
                "NO_RECOMMENDATION", [ohlcv_result.context]
            )
            if actionability.actionable:
                actionability = actionability.copy(update={
                    "actionable": False,
                    "action": "NO_RECOMMENDATION",
                    "reasons": ["OHLCV: insufficient history"],
                })
            return ScanResultItem(
                ticker=ticker, action="NO_RECOMMENDATION", confidence="—",
                mfpm_score=0, mode_w_score=0, sms_raw=0, signal_mode="MIXED",
                close=0.0, entry=0.0, sl=0.0, tp1=0.0, rr=0.0,
                amf_decision="NO_DATA", best_pattern="NONE",
                hmm_state="TRANSITIONAL", data_context=[ohlcv_result.context],
